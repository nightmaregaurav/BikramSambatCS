namespace DateConverter.Utils
{
    public abstract class WeekUtils
    {
        ...
        public static List<int?> GetWeekCalenderBs(int year, int month, int referenceDay)
        {
            ...
            var startDay = 1;
            var weekDayStart = 1;
            if (referenceDay - sundayOffset > 1)
            {
                startDay = referenceDay - sundayOffset;
            }
            else
            {
                weekDayStart = bsDate.WeekDay - (referenceDay - 1);
            }

            var weekDayEnd = 7;
            if (referenceDay + saturdayOffset > maxNumberOfDaysInMonth)
            {
                weekDayEnd = bsDate.WeekDay + (maxNumberOfDaysInMonth - referenceDay);
            }
